...
const stats = [
  {
    name: "Students from UAE, Saudi, USA, UK, Canada & more",
    icon: GlobeAltIcon,
  },
  { name: "100+ Students Taught", icon: UsersIcon },
  { name: "5+ Years Experience", icon: AcademicCapIcon },
];
        ...
        <div className="mx-auto max-w-2xl lg:max-w-none">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-y-8 gap-x-4 text-center">
            {stats.map((stat) => (
              <div
                key={stat.name}
                className="flex flex-col items-center justify-center p-4 rounded-2xl bg-white/5 border border-white/5 backdrop-blur-sm"
              >
                <stat.icon
