...
const typeWriter = (phrases, phraseIndex, charIndex, setter, erasingState = false) => {
  ...
  if (!erasingState) {
    ...
    if (charIndex < currentPhrase.length) {
      ...
      setTimeout(() => typeWriter(phrases, phraseIndex, charIndex + 1, setter, erasingState), 100);
    } else {
      // Start erasing after a delay
      setTimeout(() => typeWriter(phrases, phraseIndex, charIndex, setter, true), 2000);
    }
  } else {
    // Erasing the current phrase
    if (charIndex > 0) {
      setter(currentPhrase.substring(0, charIndex - 1));
      setTimeout(() => typeWriter(phrases, phraseIndex, charIndex - 1, setter, erasingState), 50);
    } else {
      // Move to the next phrase
      const nextPhraseIndex = (phraseIndex + 1) % phrases.length;
      setTimeout(() => typeWriter(phrases, nextPhraseIndex, 0, setter, false), 500);
    }
  }
};
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div className="relative animate-fade-in-up">
              ...
              <p className="text-xl text-gray-600 mb-6 animate-fade-in-up animation-delay-600 leading-relaxed">
                Struggling with college programming courses? Our <strong className="text-blue-700 font-semibold">daily live classes</strong> provide the practical skills your college doesn't teach. Build real projects, ask questions in real-time, and start coding confidently.
              </p>
              ...
              <div className="mt-12">
                <div className="flex flex-wrap justify-between items-center py-4 px-6 bg-gradient-to-t from-indigo-50 to-white rounded-xl border border-indigo-200 shadow-sm">
                  <div className="text-left">
                    ...
                    <p className="text-sm text-gray-500">Python 3.12: Modern Features & Development Environments</p>
                  </div>
                  <div className="flex items-center gap-2 mt-3 sm:mt-0">
                    <div className="bg-white w-12 h-14 flex flex-col items-center justify-center rounded-lg shadow-sm border border-indigo-100">
                      <span className="text-lg font-bold text-indigo-700">{countdown.hours}</span>
                      <span className="text-xs text-gray-500">hours</span>
                    </div>
                    <div className="text-indigo-400 font-bold">:</div>
                    <div className="bg-white w-12 h-14 flex flex-col items-center justify-center rounded-lg shadow-sm border border-indigo-100">
                      <span className="text-lg font-bold text-indigo-700">{countdown.minutes}</span>
                      <span className="text-xs text-gray-500">mins</span>
                    </div>
                    <div className="text-indigo-400 font-bold">:</div>
                    <div className="bg-white w-12 h-14 flex flex-col items-center justify-center rounded-lg shadow-sm border border-indigo-100">
                      <span className="text-lg font-bold text-indigo-700">{countdown.seconds}</span>
                ...
                <div className="flex flex-col space-y-4 mt-6">
                  <div className="bg-gradient-to-br from-indigo-50 to-white p-5 rounded-xl shadow-sm border border-indigo-100">
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        <h4 className="text-lg font-bold text-gray-900">LIVE Daily Classes</h4>
                        <p className="text-sm text-gray-600">Interactive sessions with real-time feedback</p>
                      </div>
                    </div>
                  </div>
                ...
                <div className="relative z-10 bg-white p-6 rounded-2xl shadow-xl border border-gray-100">
                  ...
                  >
                    ...
                    <div className="absolute inset-0 bg-black bg-opacity-20 flex items-center justify-center group-hover:bg-opacity-30 transition-all duration-300">
                      <div className={`w-16 h-16 rounded-full bg-white bg-opacity-90 flex items-center justify-center shadow-lg transition-all duration-500 ${isVideoHovered ? 'scale-110 bg-indigo-600' : 'scale-100'}`}>
                        <svg className={`w-8 h-8 ml-1 transition-colors duration-500 ${isVideoHovered ? 'text-white' : 'text-blue-600'}`} fill="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z" />
                        </svg>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-4 gap-3">
                      {['Google', 'Microsoft', 'Amazon', 'Meta'].map((company, i) => (
                        <div key={i} className="bg-indigo-50 p-2 rounded-lg text-center">
                          <p className="text-xs font-semibold text-indigo-700">{company}</p>
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  <div className="mt-4 bg-white bg-opacity-70 backdrop-blur-sm rounded-lg p-3 border border-gray-100 shadow-sm">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-700 italic">"This bootcamp changed my life! I landed a job at Google just 3 months after completing the Python course."</p>
                        <p className="text-xs font-semibold text-indigo-800 mt-1">- Priya S., Software Developer</p>
                      </div>
                    </div>
                  </div>
                </div>
